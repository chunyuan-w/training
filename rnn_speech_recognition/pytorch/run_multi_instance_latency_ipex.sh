...
for i in $(seq 1 $LAST_INSTANCE); do
    ...
    LOG_i=latency_log_bs${BATCH_SIZE}_ins${i}.txt
    ...
    numactl --physcpubind=$start_core_i-$end_core_i --membind=$numa_node_i python -u inference.py \
     $ARGS $CONFIG_FILE --val_manifest $VAL_DATASET --model_toml configs/rnnt_ckpt.toml --batch_size $BATCH_SIZE --seed $SEED --warm_up 10 --steps 100 2>&1 | tee $LOG_i &
done

numa_node_0=0
start_core_0=0
end_core_0=`expr $CORES_PER_INSTANCE - 1`
LOG_0=latency_log_bs${BATCH_SIZE}_ins0.txt
...
numactl --physcpubind=$start_core_0-$end_core_0 --membind=$numa_node_0 python -u inference.py \
     $ARGS $CONFIG_FILE --val_manifest $VAL_DATASET --model_toml configs/rnnt_ckpt.toml --batch_size $BATCH_SIZE --seed $SEED --warm_up 10 --steps 100 2>&1 | tee $LOG_0

sleep 10
throughput=$(grep 'Throughput:' ./latency_log* |sed -e 's/.*Throughput//;s/[^0-9.]//g' |awk '
BEGIN {
        sum = 0;
        i = 0;
}
...
unset thp_per_socket
thp_per_socket=$(awk -v thp="${throughput}" -v instance_per_socket="${INSTANCES_PER_SOCKET}" 'BEGIN{print (thp*instance_per_socket)}')

echo ""RNN-T";"lantency";${precision};${BATCH_SIZE};${thp_per_socket}" | tee -a ${work_space}/summary.log 
